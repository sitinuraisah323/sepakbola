...
// function chartOs() {
    ...
    $("#closemodal").on('click', function () {
        ...
        const openModal = (date, units) => {
            axios.get(`<?php echo base_url();?>/api/dashboard/Repayment_bydate/${date}/${units}`).then(res => {
                            const {
                                data
                            } = res.data;
...
// All OS
    axios.get(`<?php echo base_url();?>/api/dashboard/getRepayment`).then(res => {
        const {
            data
        } = res.data;
        const build = [];
    ...
    // AllNoa
    axios.get(`<?php echo base_url();?>/api/dashboard/getRepayment`).then(
        res => {
            const {
                data
            } = res.data;
...
function getArea(){
    ...
    // AreaNoa
    axios.get(`<?php echo base_url();?>/api/dashboard/getRepaymentArea/${selectArea}`).then(
        res => {
            const {
                data
            } = res.data;
...
var noaCabang = document.getElementById('noaCabang');
    ...
    // UnitsNoa
    axios.get(`<?php echo base_url();?>/api/dashboard/getRepaymentCabang/${selectArea}/${selectBox}`)
        .then(res => {
            const {
                data
            } = res.data;
    ...
    axios.get(`<?php echo base_url();?>/api/dashboard/getRepaymentSelectUnits/${units}`).then(res => {
        const {
            data
        } = res.data;
        const build = [];
            ...
            item => {
                ...
                const reg = a > -1 ? reguler[a] : {
                    ...
                    click: function(e) {
                        ...
                        window.open('<?php echo base_url();?>/monitoring/repayment/detail/' + date + '/' + units, '_blank');
                        win.focus(); 
                    },
                    dataPoints: []
                }
                ...
                reg.dataPoints.push({
                    x: new Date(item.repayment_date),
                    y: +item.os_reguler
                });
                ops.dataPoints.push({
                    x: new Date(item.repayment_date),
